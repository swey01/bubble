...
+function () {
	...
	function Bubble() {
		...
		for (var i = 0; i < 4; i++) {
			...
			if (i == 3) {
				if (_IE6_)
					sty.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src=heart1.png)";
				else
			...
			if (STD) {
				// sty.backgroundImage = "url(http://www.etherdream.com/funnyscript/bubbles/ch" + i + ".png)";
				arrFlt[i] = sty;
			}
			else {
				// sty.filter = "alpha progid:DXImageTransform.Microsoft.AlphaImageLoader(src=http://www.etherdream.com/funnyscript/bubbles/ch" + i + ".png)";
